...
## Spell: {name}
**Type:** spell
**Tags:** spell, high-magic, level-{level}
...
def generate_all_spells_markdown():
    """Generate markdown for all High Magic spells."""
    output = []
    for name, level, description in HIGH_MAGIC_SPELLS:
...
def post_spell_to_api(name, level, description, api_url, api_key, ruleset_id):
    ...
    payload = {
        ...
        "type": "spell",
        "tags": ["spell", "high-magic", f"level-{level}"],
        "meta_data": {"school": "High Magic", "spell_level": level},
...
def post_all_spells(api_url="http://localhost:8000", api_key="dev-secret-key", ruleset_id=1):
    """POST all High Magic spells to the API."""
    success_count = 0
    fail_count = 0
    ...
    print(f"Total spells: {len(HIGH_MAGIC_SPELLS)}")
