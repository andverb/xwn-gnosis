// Unit tests for static/js/faction-tracker/rules.js
// Run: node --test tests/faction-tracker/rules.test.js
//
...
test('calcTreasureIncome rounds up wealth/2 + (force+cunning)/4', () => {
    // force=3, wealth=4, cunning=5  →  4/2 + (3+5)/4 = 2 + 2 = 4
    assert.equal(R.calcTreasureIncome({ force: 3, wealth: 4, cunning: 5 }), 4);
    // force=2, wealth=3, cunning=2  →  1.5 + 1 = 2.5  → ceil 3
...
test('planUpkeep pays what it can, flags rest', () => {
    ...
    assert.equal(plan.lines.length, 2); // free is skipped (upkeep 0)
...
test('getExcessAssets reports over-cap attrs', () => {
    const faction = {
        ...
        assets: [
            { type: 'force', assetId: 'thugs' },
            { type: 'force', assetId: 'thugs' }, // 2 force vs cap 1 → excess 1
            { type: 'wealth', assetId: 's' },     // 1 vs cap 3 → fine
        ],
    };
...
test('calcRepairAmount: faction = ceil((highest+lowest)/2)', () => {
    ...
    // highest=5, lowest=2 → (5+2)/2 = 3.5 → ceil 4
...
test('calcRepairAmount: asset = ceil(matching attr / 2)', () => {
    ...
    assert.equal(R.calcRepairAmount({ type: 'force' }, faction), 3);  // ceil(5/2)
    assert.equal(R.calcRepairAmount({ type: 'wealth' }, faction), 2); // 4/2
    assert.equal(R.calcRepairAmount({ type: 'cunning' }, faction), 2); // ceil(3/2)
});
...
test('eligibleAssetsToBuy filters by attribute, magic, cap', () => {
    const faction = {
        ...
        assets: [
            { type: 'force', assetId: 'thugs' },
            { type: 'force', assetId: 'thugs' }, // already at cap on force
        ],
    };
    const all = [
        { type: 'force', attribute_req: 1, magic_req: 'none', name: 'A' }, // capped
        { type: 'wealth', attribute_req: 2, magic_req: 'none', name: 'B' }, // attr_req 2 vs wealth 1 → no
        { type: 'cunning', attribute_req: 1, magic_req: 'none', name: 'C' }, // ok
        { type: 'cunning', attribute_req: 1, magic_req: 'high', name: 'D' }, // magic too high
        { type: 'cunning', attribute_req: 1, magic_req: 'low', name: 'E' }, // ok
    ];
...
test('rollAttack: hit on strict greater', () => {
    let i = 0;
    const seq = [10, 1]; // atk=10+atk, def=1+def
